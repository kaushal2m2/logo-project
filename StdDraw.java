...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    public static void setPenRadius(final double radius) {
        ...
        penRadius = radius;
        final float scaledPenRadius = (float) (radius * DEFAULT_SIZE);
        final BasicStroke stroke = new BasicStroke(scaledPenRadius, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
    ...
    /**
     * Draws one pixel at (x, y). This method is private because pixels depend on
     * the display. To achieve the same effect, set the pen radius to 0 and call
     * {@code point()}.
     *
    ...
    /**
     * Draws a point centered at (x, y). The point is a filled circle whose radius
     * is equal to the pen radius. To draw a single-pixel point, first set the pen
     * radius to 0.
     *
    ...
    public static void point(final double x, final double y) {
        ...
        final float scaledPenRadius = (float) (r * DEFAULT_SIZE);
    ...
    public static void circle(final double x, final double y, final double radius) {
        ...
        final double xs = scaleX(x);
        final double ys = scaleY(y);
        final double ws = factorX(2 * radius);
        final double hs = factorY(2 * radius);
        if (ws <= 1 && hs <= 1)
            pixel(x, y);
        else
     ...
     *                                  {@code semiMinorAxis} is negative
     */
    public static void filledEllipse(final double x, final double y, final double semiMajorAxis,
    ...
    /**
     * Draws a circular arc of the specified radius, centered at (x, y), from angle1
    ...
    public static void square(final double x, final double y, final double halfLength) {
        ...
        final double xs = scaleX(x);
        final double ys = scaleY(y);
        final double ws = factorX(2 * halfLength);
        final double hs = factorY(2 * halfLength);
        if (ws <= 1 && hs <= 1)
            pixel(x, y);
        else
     ...
     *                                  {@code halfHeight} is negative
     */
    public static void filledRectangle(final double x, final double y, final double halfWidth,
    ...
    /**
     * Draws a polygon with the vertices (x0, y0), (x1, y1),
     *
     ...
     */
    /**
     * Draws the specified image centered at (x, y). The supported image formats are
     * JPEG, PNG, and GIF. As an optimization, the picture is cached, so there is no
     * performance penalty for redrawing the same image multiple times (e.g., in an
    ...
    /**
     * Draws the specified image centered at (x, y), rotated given number of
    ...
    /**
     * Draws the specified image centered at (x, y), rescaled to the specified
     ...
     * @throws IllegalArgumentException if the image filename is invalid
     */
    public static void picture(final double x, final double y, final String filename, final double scaledWidth,
    ...
    /**
     * Draws the specified image centered at (x, y), rotated given number of
     * degrees, and rescaled to the specified bounding box. The supported image
     ...
     */
    public static void picture(final double x, final double y, final String filename, final double scaledWidth,
            final double scaledHeight, final double degrees) {
        if (scaledWidth < 0)
            throw new IllegalArgumentException("width is negative: " + scaledWidth);
        if (scaledHeight < 0)
            throw new IllegalArgumentException("height is negative: " + scaledHeight);
    ...
    public static void save(final String filename) {
        ...
        // http://archives.java.sun.com/cgi-bin/wa?A2=ind0404&L=java2d-interest&D=0&P=2727
            ...
            final WritableRaster raster = onscreenImage.getRaster();
            WritableRaster newRaster;
            newRaster = raster.createWritableChild(0, 0, width, height, 0, 0, new int[] { 0, 1, 2 });
            final DirectColorModel cm = (DirectColorModel) onscreenImage.getColorModel();
